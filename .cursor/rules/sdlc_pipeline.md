...
2) **Artifact-first context (minimize chat)**
- In Cursor, pass agents the **artifact files** from `runs/...` (or `@` file references), plus only the minimal instruction needed.
- Avoid pasting long human conversations; they dilute context and cause drift.
- If extra notes are needed, put them under a clear `<context>` or `<notes>` tag and keep them short.

3) **Always validate against schemas**
...
5) **One-message mode: persist incrementally**
...
8) **Create Rails/Laravel projects via Docker when local tools aren't installed**
...
10) **Spec-driven development enforcement**
...
11) **Codebase analysis before specs**
...
13) **Fast-track mode for small tickets**
